import { BigNumber as OldBigNumber, ZERO, INFINITY, scale, bnum } from '../utils/bignumber';
import { getOutputAmountSwap } from '../pools';
import { INFINITESIMAL } from '../config';
import { NewPath, SwapTypes, PoolBase, PoolPairBase, PoolTypes } from '../types';
import { BigNumber, formatFixed, parseFixed } from '@ethersproject/bignumber';

const MINIMUM_VALUE = bnum('0.000000000000000001');

export function getHighestLimitAmountsForPaths(paths: NewPath[], maxPools: number): BigNumber[] {
...
function getAmounts(path: NewPath, swapType: SwapTypes, amount: OldBigNumber): OldBigNumber[] {
...
function getProdsFirstSpotPrices(
  ...
  for (let i = 0; i < pools.length; i++) {
    ans.push(
      getSpotPriceAfterSwap(pools[i], poolPairData[i], swapType, amounts[i + 1]).times(
        ans[ans.length - 1],
      ),
    );
  }
...
export function getDerivativeSpotPriceAfterSwapForPath(
  ...
  if (swapType === SwapTypes.SwapExactIn) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      newTerm = newTerm.times(prodsSpotPrices[i + 1]).times(prodsFirstSpotPrices[i]);
...
export function EVMgetOutputAmountSwap(
  ...
  pool.updateTokenBalanceForPool(
    tokenIn,
    balanceIn.add(
      parseFixed(amountIn.dp(poolPairData.decimalsIn).toString(), poolPairData.decimalsIn),
    ),
  );
  pool.updateTokenBalanceForPool(
    tokenOut,
    balanceOut.sub(
      parseFixed(amountOut.dp(poolPairData.decimalsOut).toString(), poolPairData.decimalsOut),
    ),
  );
...
export function takeToPrecision18(amount: BigNumber, decimals: number): BigNumber {
...
export function restorePrecision(amount: BigNumber, decimals: number): BigNumber {
  for (let i = 0; i < 18 - decimals; i++) {
