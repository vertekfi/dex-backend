...
export const optimizeSwapAmounts = (
  ...
  for (let b = initialNumPaths; b <= paths.length; b++) {
    ...
    const { paths: selectedPaths, swapAmounts: bestAmounts } = optimizePathDistribution(
      paths,
      swapType,
      totalSwapAmount,
      swapAmounts,
  ...
  sortedSwapAmounts.forEach((swapAmount) => {
    ...
    if (bestPathIndex === -1) {
      ...
      selectedPathExceedingAmounts.push(
        swapAmount.minus(bnum(formatFixed(paths[bestPathIndex].limitAmount, inputDecimals))),
      );
...
function iterateSwapAmountsApproximation(
  ...
  swapAmounts.forEach((swapAmount, i) => {
    ...
    if (
      (iterationCount == 0 && swapAmount.gte(ZERO) && exceedingAmounts[i].lte(ZERO)) ||
      (iterationCount != 0 && swapAmount.gt(ZERO) && exceedingAmounts[i].lt(ZERO))
    ) {
  ...
  const weighted_average_SPaS = bnum(
    sumSPaSDividedByDerivativeSPaSs.toNumber() / sumInverseDerivativeSPaSs.toNumber(),
  );
  ...
  swapAmounts.forEach((swapAmount, i) => {
    if (
      ...
      const deltaSwapAmount = weighted_average_SPaS.minus(SPaSs[i]).div(derivativeSPaSs[i]);
...
export const calcTotalReturn = (
  ...
  paths.forEach((path, i) => {
    totalReturn = totalReturn.plus(
      getOutputAmountSwapForPath(path, swapType, swapAmounts[i], inputDecimals),
    );
  });
  return totalReturn;
};
