...
export function filterPoolsOfInterest(
  ...
  console.log('SOR -> filterPoolsOfInterest');
        ...
        if (pool.isLBP) {
          ...
          if (raisingTokenIn) {
            ...
            if (raisingTokenIn !== wethAddress) {
              const bestRaisingTokenInToWeth = getHighestLiquidityPool(
                raisingTokenIn,
                wethAddress,
                poolsAllDict,
              );
          ...
          if (raisingTokenOut) {
            ...
            if (raisingTokenOut !== wethAddress) {
              const bestWethToRaisingTokenOut = getHighestLiquidityPool(
                wethAddress,
                raisingTokenOut,
                poolsAllDict,
              );
...
export function getHighestLiquidityPool(
  ...
  for (const id in poolsOfInterest) {
    ...
    if (!tokenListSet.has(tokenIn.toLowerCase()) || !tokenListSet.has(tokenOut.toLowerCase()))
      continue;
