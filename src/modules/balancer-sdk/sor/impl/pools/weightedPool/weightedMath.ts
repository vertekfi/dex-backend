...
export function _calculateInvariant(normalizedWeights: bigint[], balances: bigint[]): bigint {
  /**********************************************************************************************
    // invariant               _____                                                             //
...
export function _calcDueProtocolSwapFeeBptAmount(
  ...
  return denominator == BZERO ? BZERO : MathSol.divDownFixed(numerator, denominator);
}
...
export function _spotPriceAfterSwapBptOutGivenExactTokenInBigInt(
  ...
  const feeFactor =
    MathSol.ONE - MathSol.mulDownFixed(MathSol.complementFixed(weightIn), swapFeeRatio);
...
export function _spotPriceAfterSwapExactBPTInForTokenOut(
  ...
  return bnum(((1 - Aibpt / Bbpt) ** ((-1 + wo) / wo) * Bbpt * (1 + f * (-1 + wo)) * wo) / Bo);
}
...
export function _derivativeSpotPriceAfterSwapExactBPTInForTokenOut(
  ...
  const Aibpt = amount.toNumber();
  const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
  return bnum(-(((1 + f * (-1 + wo)) * (-1 + wo)) / ((1 - Aibpt / Bbpt) ** (1 / wo) * Bo)));
}
