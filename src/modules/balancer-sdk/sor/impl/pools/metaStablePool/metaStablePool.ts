...
export class MetaStablePool implements PoolBase {
  ...
  _tokenInForExactTokenOut(
    ...
    try {
      ...
      const scalingFactorIn =
        poolPairData.tokenInPriceRate.toBigInt() * BigInt(10 ** (18 - decimalsIn));

      const scalingFactorOut =
        poolPairData.tokenOutPriceRate.toBigInt() * BigInt(10 ** (18 - decimalsOut));
  ...
  _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(
    ...
    const priceRateOut = formatFixed(poolPairData.tokenOutPriceRate, 18);
    return _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(amount, poolPairData).times(
      priceRateOut,
    );
  }
