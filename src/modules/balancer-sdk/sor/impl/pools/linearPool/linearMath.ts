...
export function _calcWrappedInPerBptOut(
  ...
  const newWrappedBalance = MathSol.divUpFixed(
    MathSol.divUp(MathSol.mul(newBptBalance, previousInvariant), bptSupply) - nominalMain,
    params.rate,
  );
...
export function _spotPriceAfterSwapMainOutPerBptIn(
  ...
  return MathSol.divUpFixed(
    MathSol.ONE,
    MathSol.mulUpFixed(poolFactor, leftDerivativeFromNominal(afterNominalMain, params)),
  );
}
