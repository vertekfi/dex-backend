...
export class LinearPool implements PoolBase {
  ...
  static fromPool(pool: SubgraphPoolBase): LinearPool {
    if (pool.mainIndex === undefined) throw new Error('LinearPool missing mainIndex');
    if (pool.wrappedIndex === undefined) throw new Error('LinearPool missing wrappedIndex');
    if (!pool.lowerTarget) throw new Error('LinearPool missing lowerTarget');
    if (!pool.upperTarget) throw new Error('LinearPool missing upperTarget');
    ...
    if (swapType === SwapTypes.SwapExactIn) {
      if (linearPoolPairData.pairType === PairTypes.MainTokenToBpt) {
        return this._mainTokenInForExactBPTOut(
          poolPairData,
          balanceOutHuman.times(this.ALMOST_ONE.toString()).div(ONE.toString()),
        );
      } else if (linearPoolPairData.pairType === PairTypes.WrappedTokenToBpt) {
      ...
      } else if (linearPoolPairData.pairType === PairTypes.BptToMainToken) {
      ...
      } else if (linearPoolPairData.pairType === PairTypes.BptToWrappedToken) {
        ...
        linearPoolPairData.pairType === PairTypes.MainTokenToWrappedToken ||
        linearPoolPairData.pairType === PairTypes.WrappedTokenToMainToken
      ) {
        const limit = bnum(poolPairData.balanceOut.mul(this.ALMOST_ONE).div(ONE).toString());
      ...
      if (
        ...
        const limit = bnum(poolPairData.balanceOut.mul(this.MAX_RATIO).div(ONE).toString());
        return scale(limit, -poolPairData.decimalsOut);
      } else if (
  ...
  _exactTokenInForTokenOut(poolPairData: LinearPoolPairData, amount: OldBigNumber): OldBigNumber {
    ...
    } else if (poolPairData.pairType === PairTypes.MainTokenToWrappedToken) {
      return this._exactMainTokenInForWrappedOut(poolPairData, amount);
    } else if (poolPairData.pairType === PairTypes.WrappedTokenToMainToken) {
