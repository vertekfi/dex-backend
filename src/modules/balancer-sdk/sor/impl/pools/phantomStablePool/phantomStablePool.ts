...
type PhantomStablePoolToken = Pick<SubgraphToken, 'address' | 'balance' | 'decimals' | 'priceRate'>;
...
export class PhantomStablePool implements PoolBase {
  ...
  _calcTokensOutGivenExactBptIn(bptAmountIn: BigNumber): BigNumber[] {
    ...
    const balancesEvm = this.tokens
      ...
      .map(({ balance, priceRate, decimals }) =>
        parseFixed(balance, 18).mul(parseFixed(priceRate, decimals)).div(ONE).toBigInt(),
      );
  ...
  _calcBptOutGivenExactTokensIn(amountsIn: BigNumber[]): BigNumber {
    try {
      ...
      const balancesEvm = this.tokens
        .filter((t) => !isSameAddress(t.address, this.address))
        .map(({ balance, priceRate, decimals }) =>
          parseFixed(balance, decimals).mul(parseFixed(priceRate, 18)).div(ONE).toBigInt(),
        );
  ...
  _spotPriceAfterSwapExactTokenInForTokenOut(
    ...
    if (poolPairData.pairType === PairTypes.TokenToBpt) {
      ...
      result = phantomStableMath._spotPriceAfterSwapExactTokenInForTokenOut(
        amountConverted,
        poolPairData,
      );
    }
  ...
  _spotPriceAfterSwapTokenInForExactTokenOut(
    ...
    if (poolPairData.pairType === PairTypes.TokenToBpt) {
      ...
      result = phantomStableMath._spotPriceAfterSwapTokenInForExactTokenOut(
        amountConverted,
        poolPairData,
      );
    }
  ...
  _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(
    ...
    let result: OldBigNumber;
    if (poolPairData.pairType === PairTypes.TokenToBpt) {
      result = phantomStableMath._derivativeSpotPriceAfterSwapExactTokenInForBPTOut(
        amountConverted,
        poolPairData,
      );
    } else if (poolPairData.pairType === PairTypes.BptToToken) {
      result = phantomStableMath._derivativeSpotPriceAfterSwapExactBPTInForTokenOut(
        amountConverted,
        poolPairData,
      );
    } else {
      result = phantomStableMath._derivativeSpotPriceAfterSwapExactTokenInForTokenOut(
        amountConverted,
        poolPairData,
      );
    }
  ...
  _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(
    ...
    let result: OldBigNumber;
    if (poolPairData.pairType === PairTypes.TokenToBpt) {
      result = phantomStableMath._derivativeSpotPriceAfterSwapTokenInForExactBPTOut(
        amountConverted,
        poolPairData,
      );
    } else if (poolPairData.pairType === PairTypes.BptToToken) {
      result = phantomStableMath._derivativeSpotPriceAfterSwapBPTInForExactTokenOut(
        amountConverted,
        poolPairData,
      );
    } else {
      result = phantomStableMath._derivativeSpotPriceAfterSwapTokenInForExactTokenOut(
        amountConverted,
        poolPairData,
      );
    }
